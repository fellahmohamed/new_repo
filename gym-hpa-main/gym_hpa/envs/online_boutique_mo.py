...
class OnlineBoutiqueMO(gym.Env):
    """Horizontal Scaling for Online Boutique in Kubernetes - an OpenAI gym environment"""

    metadata = {'render.modes': ['human', 'ansi', 'array']}

    def __init__(self, k8s=False, goal_reward="cost_latency", waiting_period=0.3, objective_weights=[0.5, 0.5]):
        # Define action and observation space
        # They must be gym.spaces objects

        super(OnlineBoutiqueMO, self).__init__()
    ...
    def step(self, action):
        ...
        reward_MO = self.scalarize_reward([reward[0], reward[1]], [w1, w2])
        ...
        logging.info('[Step {}] | Action (Deployment): {} | Action (Move): {} | Reward: {} | Total Cost Reward: {} | Total Latency Reward: {}'.format(
            self.current_step, DEPLOYMENTS[action[0]], MOVES[action[1]], reward_MO, self.total_cost_reward, self.total_latency_reward))
        ...
        return np.array(ob), reward_MO, self.episode_over, self.info    
